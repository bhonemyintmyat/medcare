...
(function () {
  ...
  /* The address of the screen that edits this row. Two shapes, because
     ...
     are in the editor area, which admits editors and admins alike, and
     both roles can now save on either — see
     supabase_footer_pages_editors.sql. */
  function editHref(t) {
    if (t.kind === 'page') {
  ...
  Promise.all([target.ready, auth.ready]).then(function (results) {
    ...
    /* Every staff member gets the same link and the same wording, so
       there is nothing to repaint when a role arrives late — only a
